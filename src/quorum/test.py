...
class QUORUM(unittest.TestCase):
    ...
    def test05_run_client_read_nr(self):
        ...
        assert (
            latest_file.name.find(self.client_files[0][0][1]) != -1
        ), f"Filename mismatch, got {latest_file.name}"
    ...
    def test07_run_client_read_nr_two(self):
        ...
        assert latest_file.status == "SUCCESS", "Read failed"
        assert (
            latest_file.name.find(self.client_files[0][1][1]) != -1
        ), f"Filename mismatch, got {latest_file.name}"
    ...
    def test08_run_client_delete_one(self):
        ...
        assert (
            count_success >= self.nw + self.nr - self.n
        ), f"Delete failed on {self.nw-count_success} replicas out of {self.nw}"
